...
export class DocxSession {
  ...
  static parseTitleToEntry(
    text: string,
  ): Pick<DocxSession, 'level' | 'deepLevel' | 'title'> | null {
  ...
  private static titleElement(
    nodeEl: Cheerio<Element>,
  ...
  private static titleParagraphElement(
    nodeEl: Cheerio<Element>,
  ): Cheerio<Element> | null {
  ...
  is(nodeEl: Cheerio<Element>): boolean {
    ...
    return text.slice(0, 100).replaceAll(' ', '').startsWith(
      this.toTitle().replaceAll(' ', ''),
    )
  }
...
function splitParagraphsFromSession(
  ...
  return sessions.map((session, index) =>
    new DocxParagraph(session.id, paragraphsBySession[index] ?? [])
  )
}
