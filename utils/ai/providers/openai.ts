...
export class OpenAiProvider implements AiProvider {
  ...
  loadConfigFromEnv(overrides: Partial<ModelConfigParam> = {}): ModelConfig {
    ...
    const temperature = overrides.temperature ??
      parseNumber(process.env.AI_TEMPERATURE, undefined, 'AI_TEMPERATURE')
  ...
  async streamingInvokeIterator(agent: Agent, beforeMessages: BaseMessage[]) {
    const stream = await agent.stream({ messages: beforeMessages }, {
      streamMode: 'values',
    })

    return async function* () {
      for await (
        const chunk of stream as AsyncIterable<{ messages?: BaseMessage[] }>
      ) {
