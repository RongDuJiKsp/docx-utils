...
function buildSystemPrompt() {
  return [
    ...
    '   - 每次只审查一个章节，禁止同时处理多个章节。',
    '   - 将章节正文按换行拆分为段落，按顺序逐段检查，不得跳过。',
    '   - 每段至少对照一次规则，发现问题立即调用 record_issue。',
    '   - 当前章节未完成前，禁止调用下一章节。',
...
export async function aiReview(fileName: string, options: AiReviewOptions) {
  ...
  for (const session of document.sessions) {
    console.log(
      `章节: ${session.id}, 标题: ${session.toTitle()}, 层级: ${session.level}`,
    )
  }
  ...
  const maxIterations = Math.max(
    ...
    options.maxIterations ??
      parseNumber(
        process.env.AI_REVIEW_MAX_ITERATIONS,
        40,
        'AI_REVIEW_MAX_ITERATIONS',
      ),
  )
  ...
  const iteratorFactory = await provider.streamingInvokeIterator(
    agent,
    beforeMessages,
  )
